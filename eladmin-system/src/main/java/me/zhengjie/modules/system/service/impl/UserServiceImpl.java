...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public void update(User resources) throws Exception {
        ...
        user.setFavoriteGenres(resources.getFavoriteGenres());
        saveOrUpdate(user);
        // 清除缓存
    ...
    @Override
    public List<String> listFavorite(String userId) {
        ...
        if (favoriteGenres != null && !favoriteGenres.isEmpty()) {
            ...
            return dictDetailMapper.selectLabel("6", genres);
        }
        return Collections.emptyList();
    }

    @Override
    public void addFavorite(String userId, String value) {
        User user = userMapper.selectById(userId);
        String favoriteGenres = user.getFavoriteGenres();
        if (favoriteGenres != null && !favoriteGenres.isEmpty()) {
            if (!favoriteGenres.contains(value)) {
                String[] genres = favoriteGenres.split(",");
                int len = genres.length;
                String[] newGenres = new String[len + 1];
                System.arraycopy(genres, 0, newGenres, 0, len);
                newGenres[len] = value;
                favoriteGenres = String.join(",", newGenres);
            }
        } else {
    ...
    @Override
    public void removeFavorite(String userId, String value) {
        User user = userMapper.selectById(userId);
        String favoriteGenres = user.getFavoriteGenres();
        if (favoriteGenres != null && !favoriteGenres.isEmpty()) {
            String[] genres = favoriteGenres.split(",");
            StringBuilder newGenres = new StringBuilder();
            ...
            user.setFavoriteGenres(newGenres.toString());
        }
        saveOrUpdate(user);
    }
